...
$(function() {
	$.fn.smohanfacebox = function(options) {
		...
		//创建表情框
		var faceimg = '';
	    for(i=0;i<36;i++){  //通过循环创建60个表情，可扩展
		 faceimg+='<li><a href="javascript:void(0)"><img src="'+publicurl+'/images/index/face/'+(i+1)+'.gif" face="<emt>'+(i+1)+'</emt>"/></a></li>';
		 };
		$("#"+options.divid).prepend("<div id='SmohanFaceBox'><span class='Corner'></span><div class='Content' style='margin-left: 45px;'><h3><span>常用表情</span><a class='close' title='关闭'></a></h3><ul>"+faceimg+"</ul></div></div>");
...
/* 		 $('#SmohanFaceBox').mouseleave(function(){
 ...
 $.fn.extend({
	  replaceface : function(faces){
		  for(i=0;i<60;i++){
			  faces=faces.replace('<emt>'+ (i+1) +'</emt>','<img src="'+publicurl+'/images/index/face/'+(i+1)+'.gif">');
			  }
		   $(this).html(faces);
		   }
	  });
